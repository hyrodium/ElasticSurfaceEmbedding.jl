...
Settings("Q003-"*(@sprintf "%02d" id),up=3,down=-3,right=3,left=-3,mesh=(20,1),unit=200,slack=true,overwrite=true)
...
NewtonMethodIteration(parent=4)
Refinement(p₊=[0,1],k₊=[Knots([]),Knots([(id-1/2)/10])],parent=9)
...
ElasticSurfaceEmbedding.hoge
ElasticSurfaceEmbedding.dict
