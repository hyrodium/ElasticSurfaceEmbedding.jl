...
function MESH2Export(p,D;name="MESH2Export.inc",mesh=(10,10),smooth=true,texture=nothing)
    ...
    P=[p([t1,t2])[i] for t1 ∈ t[1], t2 ∈ t[2], i ∈ 1:3]
    Pc=[p([t1,t2])[i] for t1 ∈ tc[1], t2 ∈ tc[2], i ∈ 1:3]
    if(smooth)
        ṗ(t)=ForwardDiff.jacobian(p,t)
        e3(t)=cross(ṗ(t)[1:3,1],ṗ(t)[1:3,2])#LinearAlgebraは不要??
        E=[e3([t1,t2])[i] for t1 ∈ t[1], t2 ∈ t[2], i ∈ 1:3]
        Ec=[e3([t1,t2])[i] for t1 ∈ tc[1], t2 ∈ tc[2], i ∈ 1:3]
    end
    if(texture!=nothing)
        colo=texture[2]
        T=[colo([t1,t2])[i] for t1 ∈ t[1], t2 ∈ t[2], i ∈ 1:3]
        Tc=[colo([t1,t2])[i] for t1 ∈ tc[1], t2 ∈ tc[2], i ∈ 1:3]
    end

    Fc=[i₁-1+mesh[1]*(i₂-1) for i₁ ∈ 1:mesh[1], i₂ ∈ 1:mesh[2]]
    F=[mesh[1]*mesh[2]+i₁-1+(mesh[1]+1)*(i₂-1) for i₁ ∈ 1:(mesh[1]+1), i₂ ∈ 1:(mesh[2]+1)]
    ...
    *(map(POVvec,reshape([Pc[i₁,i₂,:] for i₁ ∈ 1:mesh[1], i₂ ∈ 1:mesh[2]],mesh[1]*mesh[2]))...)*"\n"*
    *(map(POVvec,reshape([P[i₁,i₂,:] for i₁ ∈ 1:(mesh[1]+1), i₂ ∈ 1:(mesh[2]+1)],(mesh[1]+1)*(mesh[2]+1)))...)*"\n"*
    "}\n"
    if(smooth)
        MESH2=MESH2*
        "normal_vectors{\n"*
        string((mesh[1]+1)*(mesh[2]+1)+mesh[1]*mesh[2])*"\n"*
        *(map(POVvec,reshape([Ec[i₁,i₂,:] for i₁ ∈ 1:mesh[1], i₂ ∈ 1:mesh[2]],mesh[1]*mesh[2]))...)*"\n"*
        *(map(POVvec,reshape([E[i₁,i₂,:] for i₁ ∈ 1:(mesh[1]+1), i₂ ∈ 1:(mesh[2]+1)],(mesh[1]+1)*(mesh[2]+1)))...)*"\n"*
        "}\n"
    end
    if(texture!=nothing)
        MESH2=MESH2*
        "texture_list{\n"*
        string((mesh[1]+1)*(mesh[2]+1)+mesh[1]*mesh[2])*"\n"*
        *(map(POVtxt,reshape([Tc[i₁,i₂,:] for i₁ ∈ 1:mesh[1], i₂ ∈ 1:mesh[2]],mesh[1]*mesh[2]))...)*"\n"*
        *(map(POVtxt,reshape([T[i₁,i₂,:] for i₁ ∈ 1:(mesh[1]+1), i₂ ∈ 1:(mesh[2]+1)],(mesh[1]+1)*(mesh[2]+1)))...)*"\n"*
        "}\n"
        MESH2=MESH2*
        "face_indices{\n"*
        string(4*mesh[1]*mesh[2])*"\n"*
        *(map(POVvec2,[Fi₁[i,:] for i ∈ 1:(mesh[1]*mesh[2])])...)*"\n"*
        *(map(POVvec2,[Fi₂[i,:] for i ∈ 1:(mesh[1]*mesh[2])])...)*"\n"*
        *(map(POVvec2,[Fi₃[i,:] for i ∈ 1:(mesh[1]*mesh[2])])...)*"\n"*
        *(map(POVvec2,[Fi₄[i,:] for i ∈ 1:(mesh[1]*mesh[2])])...)*"\n"*
        "}\n"*
        ...
        "face_indices{\n"*
        string(4*mesh[1]*mesh[2])*"\n"*
        *(map(POVvec,[Fi₁[i,:] for i ∈ 1:(mesh[1]*mesh[2])])...)*"\n"*
        *(map(POVvec,[Fi₂[i,:] for i ∈ 1:(mesh[1]*mesh[2])])...)*"\n"*
        *(map(POVvec,[Fi₃[i,:] for i ∈ 1:(mesh[1]*mesh[2])])...)*"\n"*
        *(map(POVvec,[Fi₄[i,:] for i ∈ 1:(mesh[1]*mesh[2])])...)*"\n"*
        "}\n"*
